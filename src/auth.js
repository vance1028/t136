...
const { withAccessControl } = require('./permissions');

const JWT_SECRET = process.env.JWT_SECRET || 'apiary-admin-dev-secret';
...
function authRequired(req, res, next) {
  ...
  try {
    ...
    if (!user || !user.active) return sendError(res, 401, '用户不存在或已停用');
    req.user = user;
    withAccessControl(req, res, next);
  } catch (e) {
